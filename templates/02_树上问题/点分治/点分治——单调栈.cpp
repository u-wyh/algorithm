...
// 提交如下代码，可以通过所有测试用例
// 这道题的难点是如何将点对的数量降下来 
// 原来是O（n*n）级别的  经过操作之后可以降到O(n*log(n))级别  具体思路是点分治
// 首先介绍一下大思路将所有的点对按照第二维排序 所有的查询也按照第二维排序
// 处理每个查询的时候  将点对第二维小于目前值的所有点对加入到线段树  按照第一维组织  变小会更新
// 这样处理查询的时候  就可以查询l到r之间的最小值即可  总的时间复杂度是O(n*log(n)*log(n))
// 我们按照点分治的步骤 当来到当前节点的时候 将他的子树中的所有节点到他的距离全部收集起来
// 根据编号从小到大  如果一个点对(a,b)在以当前重心为头结点的时候可以被保留  那么说明从a到b区间  他们是最小和次小
// 也就是说这中间不能存在编号介于ab之间  但是dist却小于a或者b的
// 那么对于一个编号而言  只需要查询自己作为次小值时可能会组成那些点对   作为最小值的点对会被别人收集
// 那么借助单调栈实现很方便
#include <bits/stdc++.h>
using namespace std;
