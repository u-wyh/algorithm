...
// 提交如下代码，可以通过所有测试用例
// 这道题用到了一种“有效结构”：  就是如果对于一个范围  我要选择一些节点  
// 这些节点的父亲节点如果也在这个范围中  那么如果选了这个节点  则它的父亲节点也必须选择
// 这个确实就和前面的第一道题是一样的（不完全一样，但是有点类似）  但是这个是常用的结构
// 这道题利用了二进制分组来优化01背包  实际上是多重背包
#include <bits/stdc++.h>
using namespace std;
const int MAXN = 501;
const int MAXM = 4001;
...
int val[MAXN];
int cost[MAXN];
// dp[i][j]:表示选择从i到目前最大序号+1  花费j元得到的最大价值
int dp[MAXN + 1][MAXM];
...
int calc(int u) {
    ...
    for (int i = cntd; i > 0; i--) {
		...
		// 先必选一件，然后考虑更多件，因为二进制分组可能导致选择0件  但是至少得选择一件
        int cnt = d[cur] - 1;
        int num = 0;
