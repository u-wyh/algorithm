// 题目：棘手的操作
// OJ：洛谷 P3273
// 链接：https://www.luogu.com.cn/problem/P3273
...
// 这道题实际上是没有懒更新的  所有的信息全部在头结点上  不会下发
// 因为这道题有将单独一个点增加权值的操作  那么我们需要将这个点单独拿出来  在修改正确之后  重新加回去
// 但是这样的话  需要把所有的懒信息全部落实 删除一个节点是log(n) 但是要落实效果的话就是O(n)
// 所以我们不使用懒更新  而是将所有的懒信息全部放在头结点
// 那么在两个根合并的时候  我们使用启发式合并  将子树大小较小的那个全部落实效果
// 这个时间复杂度是可以接受的
#include <bits/stdc++.h>
using namespace std;
