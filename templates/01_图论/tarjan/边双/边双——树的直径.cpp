...
// 提交如下代码，可以通过所有测试用例
// 这道题的大思路是二分答案法，因为肯定是钱越多，那么成功的可能越大
// 二分一个答案，如果权值小于这个答案的边可以通过加一条边形成一个环，那么说明这个答案不行
// 因为他只需要形成一个环之后，你无论删除（权值小于答案的）那条边，他都是联通的  权值大的边删不掉
// 只要无法形成一个环，那么说明一定是至少有一条边在这个环外，那我就把这条边删掉即可
// 如何判断这些边能不能形成环呢？  利用树的直径
// 将边权小于这个二分答案的权值设置为1  否则为0
// 如果树的直径就是1的总和  说明可以形成环
// 因为如果可以被删除的边都可以在一条链上，那么必然可以通过加一条边形成环，导致失败
// 如果说不在一条链上，那么必然可以删除一条边可以满足
// 所以可以通过树的直径来判断
#include <bits/stdc++.h>
