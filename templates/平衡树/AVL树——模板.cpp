...
void up(int i) {
    sz[i] = sz[ls[i]] + sz[rs[i]] + keycount[i];
...
int add(int i, int num) {
    if (i == 0) {
        //这里表示从未出现过  赋予新编号
        key[++cnt] = num;
        keycount[cnt] = sz[cnt] = height[cnt] = 1;
        //初始化信息
        return cnt;
    }
    if (key[i] == num) {
...
int small(int i, int num) {
    ...
    if (key[i] >= num) {
        ...
        return sz[ls[i]] + keycount[i] + small(rs[i], num);
    }
}
...
int index(int i, int x) {
    if (sz[ls[i]] >= x) {
        return index(ls[i], x);
    } else if (sz[ls[i]] + keycount[i] < x) {
        return index(rs[i], x - sz[ls[i]] - keycount[i]);
    }
    return key[i];
}
