...
// 对于种类小于blen的  直接暴力查询O(n)  也就是时间复杂度为O(n*根号n)
// 对于种类大于blen的  种类越大  那么答案就会越小  而且答案的取值只会有1~根号n
// 那么我们再求出一个种类的答案后  二分检查这个答案可以延伸到哪里 然后下一个位置继续查询
// 因为相当于有 n-根号n 个种类的结果  他们会共享 根号n 个值
