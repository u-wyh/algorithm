...
// 提交如下代码，可以通过所有测试用例
// 其实加、减、乘都是比较简单的操作，无非就是利用位图（乘法是分解自己的因子，也很简单）
// 对于除法而言，如果x大于根号n的话，那么直接枚举，需要枚举的值也不是很多
// 如果小于根号n的话,那么就要用到单独处理了
// 首先把问题全部收上来,对于每个x（小于根号n）做出dp数组,dp[i]表示1~i可以满足相除为x的左边坐标的最优边界是多少
// 这样查询l到r区间是否存在的时候，就可以通过查询dp[r]来判断了
// 需要利用到pre数组
#include <bits/stdc++.h>
using namespace std;
