...
// 提交如下代码，可以通过所有测试用例
//这道题相比较上一道题要简单   因为最初的边保证不会删除  
//同时永远图都是联通的  所以每加入一条边  那么就会形成一个环
//结合位图
//这道题由于保证了联通性  那么并查集做扁平化处理  因为没有撤销并查集操作 
#include <bits/stdc++.h>
using namespace std;
const int MAXN = 501;
const int MAXQ = 1001;
...
//其实这应该传入一个参数  表示从起始点到终点的异或值
//但是这个起始点和终点都是1  也就是说这个值是0
bs maxEor() {
    bs ret;
    for (int i = BIT; i >= 0; i--) {
...
void Union(int u, int v, bs& w) {
    ...
    if (fu == fv) {
        ...
        //这个只会在初始加边时有作用
        father[fv] = fu;//每次记录自己的父亲节点  相当于建立一棵树  这样方便寻找简单路径
        eor[fv] = weight;
    }
}
...
int main() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        if (op == "Add") {
            ...
            last[edgeCnt] = i;//表示上一次对这个编号的边操作时间是i
        } else if (op == "Cancel") {
            cin >> k;
