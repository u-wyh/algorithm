...
// 提交如下代码，可以通过所有测试用例
// 某个子树左右节点交换其实不会影响外面的节点  也不会影响这两个左右子树内部的逆序对数量
// 因为对于外部而言  就算交换了  外部和这个树的相对位置也不会发生变化
// 对于左右子树内部而言  内部的结果其实是没有发生变化的
#include <bits/stdc++.h>
using namespace std;
...
// 全局变量 用于统计交换或者不交换会产生的逆序对
// 这个统计的过程是在线段树合并的过程中实现的
// 非常巧妙  时间复杂度就是合并的时间复杂度
long long no, yes;
