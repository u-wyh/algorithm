...
//轻儿子的没有影响  所以正确
// 这里理解的关键就是每个节点统计答案的时候  他的dp表的数据其实已经是对于自己长儿子这一脉的正确统计了
// 因为距离自己儿子为i的  填在了dfn[son[u]]+i  也就是dfn[u]+1+i 即距离u为i+1
void dfs2(int u, int fa) {
    ...
    // 比如 全图就是一条链  那么时间复杂度相当于O(n^2)
    // 所以长链信息会被u直接继承，绝对不要有任何与长链的长度等规模的循环出现！
    // 这里合并的时间复杂度是O(n)  原因是每条长链只会在来到父亲节点的时候被暴力合并一次
    // 也就是所有的点只会被用一次
    ansx[u] = ansx[son[u]] + 1;
    for (int e = head[u], v; e > 0; e = nxt[e]) {
        v = to[e];
