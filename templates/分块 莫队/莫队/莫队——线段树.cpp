...
// 每当B数组中增加一个值的时候 那么匹配A中的最大的符合条件的
// 每当B数组中减少一个值的时候 那么释放A中的最小的符合条件的（即使当初匹配的不一定是这个值，因为这个值可能已经被占用了）
// 而且删除的时候不能影响其他的配对
// 这道题确实很有意思，尤其是线段树
// 每个b数组对应的值其实要匹配a数组中的位置是固定的，不会因为出现了多个相同的数值就会去不同的地方
// 需要借助over数组，实现左边剩余的给右边平账用
// 这样可以保证每个match匹配对都一定是合理的  
// 因为要么是自己最开始匹配的那个最大的  要么是自己和应该匹配的值更小的  但这样都是合理的
// 在释放的时候，最应该的那个数字肯定是被匹配了（虽然可能不是自己匹配的），因为总是优先匹配这个最大的，
#include <bits/stdc++.h>
using namespace std;
