...
// 提交如下代码，可以通过所有测试用例
// 这道题的时间复杂度分析真的是特别好，用到了势能分析
// 操作的时候会根据减去的值和真实的最大值的一半进行比较
// 为什么呢？因为本质上是在不断压缩真实数据的空间
// 而且根据真实最大值的一半可以将一半数据完全压缩到另一半数据中
// 这样操作的话，数据有效范围会不断地压缩，势能分析可以保证时间复杂度不会超时
#include <bits/stdc++.h>
using namespace std;
...
// 全包的时候  我们分为两种情况来讨论   一种是大于真实最大值的一半  一种是小于真实最大值的一半
