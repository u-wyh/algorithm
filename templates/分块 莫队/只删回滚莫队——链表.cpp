...
// 提交如下代码，可以通过所有测试用例
// 这道题借助回滚莫队的特征：删除的顺序也是添加顺序的回滚
// 利用双向链表实现
// 这道题不能使用   普通莫队+双向链表  实现   根本原因就在于回滚莫队的特征：删除的顺序也是添加顺序的回滚
// 比如一开始1~25   查询4~21   那么就是 删除25 24 23 22 1 2 3  以这个顺序删除  
// 如果变成2~23 那么就是加上 3 2 1 22 23 然后删除1
// 就是说加入的顺序一定是删除时的逆序   而普通莫队则无法实现这一点  
#include <bits/stdc++.h>
using namespace std;
